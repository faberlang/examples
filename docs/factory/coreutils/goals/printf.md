...
bounded exponent-zero `%e`/`%E`, simple positive field width, and string
precision.
...
The numeric slice intentionally avoids flags, dynamic width/precision, full
exponent normalization, negative unsigned wrapping/overflow behavior,
...
- Inline `proba` cases cover plain text, `%%`, `%s`, format repetition, missing
  ...
  bounded exponent-zero `%e`/`%E`, plus simple width and string precision.
