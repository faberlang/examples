...
| `printf` | 3 | slice | — | plain text, raw no-newline output, `%%`, `%s`, repeated `%s`, `%b` basic escapes, plain `%d`/`%i` decimal integers, positive `%u`; inline proba formatting coverage | other numeric formats, widths/precision, usage diagnostics | Continue numeric formats/options or ship milestone |
