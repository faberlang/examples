...
| `printf` | 3 | slice | — | plain text, raw no-newline output, `%%`, `%s`, repeated `%s`, `%b` basic escapes, plain `%d`/`%i` decimal integers with missing numeric argument as `0`, positive `%u`, positive `%o`, positive `%x`/`%X`, simple field width, string precision; inline proba formatting coverage | floating formats, dynamic/numeric precision, usage diagnostics | Continue numeric formats/options or ship milestone |
