...
| `printf` | 3 | slice | — | plain text, raw no-newline output, `%%`, `%s`, repeated `%s`, `%b` basic escapes, plain `%d`/`%i` decimal integers with missing numeric argument as `0`, positive `%u`, positive `%o`, positive `%x`/`%X`, selected ASCII quoted-character numeric operands, bounded positive `%f` with fixed-decimal rounding, bounded exponent-zero `%e`/`%E`, simple field width, string precision, dynamic `*` width/precision including negative width left alignment; inline proba formatting coverage | full exponent normalization, usage diagnostics | Continue numeric formats/options or ship milestone |
