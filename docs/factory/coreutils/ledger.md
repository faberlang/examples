...
| `printf` | 3 | slice | — | plain text, raw no-newline output, `%%`, `%s`, repeated `%s`, `%b` basic escapes, plain `%d`/`%i` decimal integers with missing numeric argument as `0`, positive `%u`, positive `%o`, positive `%x`/`%X`, bounded positive `%f` with fixed-decimal rounding, bounded exponent-zero `%e`/`%E`, simple field width, string precision, positive dynamic `*` width/precision; inline proba formatting coverage | full exponent normalization, negative dynamic widths, usage diagnostics | Continue numeric formats/options or ship milestone |
