...
|---|---|---|
| `extract_reference.py` | Rebuilds `inputs.json` / `loss-trace.json` / `gradients.json` / `final-params.json` verbatim (stepper Display strings preserved) from a fresh `capture.txt` (S5-U7; replaces the ad-hoc 8-step generation). | the four JSON reference files |
...
python3 oracle/tools/extract_reference.py   # rebuilds the four JSON reference files
