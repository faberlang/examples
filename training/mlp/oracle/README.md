...
| `gradients.json` | Per-step gradient tensors from the CPU companion (all six slots: input, weight1, bias1, weight2, bias2, target). |
...
- `gradients.json` — `{"fixture", "rule", "method", "steps": [{"step", "gradients":
  ...
  `(grad_input, grad_weight1, grad_bias1, grad_weight2, grad_bias2, grad_target)`.
