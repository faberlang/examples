...
fn parameterized_write_query_and_scalar_round_trip() {
    ...
    exsequi(
        via.clone(),
        "CREATE TABLE message (id INTEGER PRIMARY KEY, subject TEXT NOT NULL)".to_string(),
        Vec::new(),
    )
    .expect("create table");
    let effect = exsequi(
        via.clone(),
        "INSERT INTO message(subject) VALUES (?1)".to_string(),
    ...
    let rows = quaere(
        via.clone(),
        "SELECT id, subject FROM message WHERE subject = ?1".to_string(),
        vec![Valor::Textus("salve".to_owned())],
    )
    .expect("query row");
    ...
    assert_eq!(
        scalar(
            via,
            "SELECT subject FROM message WHERE id = ?1".to_string(),
            vec![Valor::Numerus(1)],
        )
        .expect("query scalar"),
...
fn batch_commits_all_statements_on_success() {
    ...
    exsequi(
        via.clone(),
        "CREATE TABLE item (id INTEGER PRIMARY KEY, name TEXT UNIQUE NOT NULL)".to_string(),
        Vec::new(),
    )
    .expect("create table");
    ...
    assert_eq!(
        scalar(via, "SELECT COUNT(*) FROM item".to_string(), Vec::new()).expect("count rows"),
...
fn transaction_commits_all_steps() {
    ...
    exsequi(
        via.clone(),
        "CREATE TABLE item (id INTEGER PRIMARY KEY, name TEXT NOT NULL)".to_string(),
        Vec::new(),
    )
    .expect("create");
    ...
    let count = scalar(via, "SELECT COUNT(*) FROM item".to_string(), Vec::new())
        .expect("count")
        .expect("row");
...
fn transaction_rolls_back_on_step_error() {
    ...
    exsequi(
        via.clone(),
        "CREATE TABLE item (id INTEGER PRIMARY KEY, name TEXT NOT NULL UNIQUE)".to_string(),
        Vec::new(),
    )
    .expect("create");
