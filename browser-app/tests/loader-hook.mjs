...
export async function resolve(specifier, context, nextResolve) {
  ...
  if (specifier === "web:dom" || specifier === "web:web") {
    console.error("loader-hook: intercept bare web:dom/web:web → bridge");
    return { url: bridgeUrl, shortCircuit: true };
  }
  // Compiled relative path: Faber/radix compiler rewrites bare specifiers
  // (e.g. "web:dom") to relative paths (e.g. "./web-dom.js"). Catch those
  // too so the runtime bridge is used instead of the generated stubs.
  if (specifier.endsWith("/web-dom.js") || specifier.endsWith("/web-web.js")) {
    console.error("loader-hook: intercept compiled ./web-dom.js → bridge");
    return { url: bridgeUrl, shortCircuit: true };
  }
